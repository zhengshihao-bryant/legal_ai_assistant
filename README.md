...
| --- | --- | --- | --- | --- | --- |
| 法规 | 30 | **0.433** | 0.209 | 0.933 | 0.933 |
| 制度 | 38 | 0.974 | 0.172 | 0.974 | 0.983 |
| 合同 | 13 | 1.0 | 0.123 | 1.0 | 1.0 |
| 案例 | 20 | **0.45** | 0.139 | 1.0 | 1.0 |

**错误分桶**：A 检索失败 29 条 · B 证据命中但生成差 59 条（本地抽取式上限）· C 引用缺失 3 条 · D 接地性风险 3 条

**根因分析**（完整报告：[qa_class_v1.md](data/evaluation/reports/qa_class_v1.md)）：
1. **OCR 扫描件是检索短板主因之一**：R001/R030 的 gold 文档（劳动合同法/招标投标法）为扫描件，OCR 噪声弱化向量表达，检索结果被制度 FAQ 抢占
2. **lexical gap 实证**：R002/R003「加班工资」gold 是《劳动法》，但法条原文用「延长工作时间」——口语与法条术语不匹配，② 的规则词典需补充此类映射（加班→延长工作时间）
3. **制度 FAQ 抢答**：制度文档内含 Q&A 段落直接给出答案，检索器认为比法律原文更相关（答案口径一致，但溯源目标偏移）
4. **兜底生成器 3 条「找不到」**：词重叠为 0 时直接放弃，应降级返回最相关证据句（待修）
